...
class Messages:
    ...
    START_TIMEZONE_SYNC_PROMPT = (
        ...
        "🕒 Если вы переехали или сменили часовой пояс — нажмите «Продолжить», "
        "бот подстроит окна уведомлений под местное время.\n\n"
    ...
    START_EXPAND_CIRCLE_PROMPT = (
        "Расширьте круг заботы: добавьте родителей, лучшего друга или других "
    ...
    CALLBACK_SEND_PERIOD_CLOSED = (
        "⏰ Время для отправки этого пожелания уже закончилось. "
        "Дождитесь следующего уведомления от бота."
    )
    ...
    NOTIF_TIME_MORNING_PROMPT = (
        ...
        "Сейчас установлено: <b>{current_range}</b>\n\n"
        "{partner_info}"
        "Выберите интервал:"
    )
    ...
    NOTIF_TIME_PARTNER_WINDOW_INFO = (
        "У вашего партнёра в этой паре установлено время: <b>{partner_range}</b>.\n"
        "⚠️ <i>Внимание: учитывайте разницу во времени с учётом часовых поясов.</i>\n\n"
    )
    NOTIF_TIME_DONE = (
        "✅ Готово!\n\n"
    ...
    NOTIF_TIME_SELECT_PAIR_PROMPT = (
        "Выберите пару, чтобы настроить своё время и увидеть время партнёра:"
    )
    ...
    START_BOTH_DEMO_USED = (
        "❌ Демо период для этой связи уже был использован. "
        "Для продолжения использования оплатите подписку."
    )
    START_PAIR_RECONNECTED_REQUIRES_PAYMENT = (
        "✅ {partner_text} принял приглашение.\n\n"
        "❌ Демо период для этой связи уже был использован. "
        "Оплатите подписку, чтобы активировать связь."
    )

...
async def handle_request_morning(
    ...
    """Handle legacy morning request callback buttons (pre-WebApp prompts)."""
    tg_id = callback.from_user.id
    ...
    parsed = parse_callback_data_with_day(callback.data, prefix="request_morning_")
    if not parsed:
        return
    ...
    logger.info(
        "Processing legacy request_morning callback",
        pair_id=pair_id,
        user_id=user_id,
        tg_id=tg_id,
